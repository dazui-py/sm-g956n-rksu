...
def patch_readdir_func(func):
    ...
    # Melhor ponto neste kernel Samsung: antes de escrever no dirent.
    for i in range(start, end):
        if re.match(r"^\s*dirent\s*=\s*buf->previous\s*;", lines[i]):
            target = i
            break
    ...
    if target is None:
        ...
        fail(f"fs/readdir.c: anchor 'dirent = buf->previous;' not found inside {func}")
